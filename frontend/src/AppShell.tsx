...
export function AppShell({ user, token, onLogout }: { user: User; token: string; onLogout: () => void }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); void executeSimulation() }
      ...
      <div className="workspace">
        <aside className="panel controls">
          ...
          <form onSubmit={submit}>
            ...
            <button className="primary" disabled={loading}>{loading ? 'Procesando...' : 'Ejecutar simulación'} <span>→</span></button>
          </form>
        ...
        <section className="results">
          ...
          <div className="panel bridge-panel"><div className="panel-heading"><div><span className="section-kicker">02 / Visualización 3D</span><h2>Modelo de respuesta</h2></div><div className="legend"><span><i className="legend-dot critical" /> Crítica</span><span><i className="legend-dot high" /> Alta</span><span><i className="legend-dot medium" /> Media</span></div></div><BridgeScene3D zones={result.zones} monitoringPoints={result.monitoring_points} /></div>
          <div className="lower-grid"><div className="panel severity-panel"><div className="panel-heading compact"><div><span className="section-kicker">03 / Diagnóstico</span><h2>Zonas por severidad</h2></div></div>{result.zones.map((zone) => <div className="severity-row" key={zone.label}><span className={`severity-bar ${zone.severity}`} /><strong>{zone.label}</strong><span className="severity-type">{severityLabels[zone.severity]}</span><span className="severity-value">{zone.displacement.toFixed(3)} m</span></div>)}</div><div className="panel recommendation"><span className="section-kicker">04 / Instrumentación</span><h2>Ubicaciones recomendadas</h2>{result.monitoring_points.map((point, index) => <div className="recommendation-row" key={`${point.position}-${index}`}><span className="sensor-badge">{String(index + 1).padStart(2, '0')}</span><div><strong>{point.reason}</strong><small>Posición {Math.round(point.position * 100)}% del tablero</small></div><span className="priority">{point.priority === 'high' ? 'Prioridad alta' : 'Recomendada'}</span></div>)}{result.simulation_id !== 'demo' && <div className="export-actions"><button type="button" onClick={() => void exportSimulation(result.simulation_id, token, 'html')}>Informe HTML</button><button type="button" onClick={() => void exportSimulation(result.simulation_id, token, 'json')}>Exportar JSON</button><button type="button" onClick={() => void exportSimulation(result.simulation_id, token, 'csv')}>Exportar CSV</button></div>}</div></div>
        </section>
      </div>
