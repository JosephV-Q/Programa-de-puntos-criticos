  ...
  return (
    <div className="user-panel" role="dialog" aria-label="Panel de usuario">
      <div className="user-panel-heading">
        <div>
          <span className="section-kicker">Cuenta activa</span>
          <h2>Tu perfil</h2>
        </div>
        <button
      ...
      <div className="profile-detail">
        ...
        <strong>{new Date(user.created_at).toLocaleDateString("es-CO")}</strong>
      </div>
      {user.role === "admin" && (
          ...
          {users.map((item) => (
            <div className="admin-user" key={item.id}>
              ...
              <select
                value={item.role}
                onChange={(event) =>
                  void changeRole(item.id, event.target.value as UserRole)
                }
