...
const pylonConfigs: PylonConfig[] = [
  {
    z: -146,
    groundY: DECK_LEVEL - 54,
    totalHeight: 112,
    label: "Pilono Norte · 112 m",
  },
  {
    z: 146,
    groundY: DECK_LEVEL - 74,
    totalHeight: 132,
    label: "Pilono Sur · 132 m",
  },
];
...
function responseScoreAt(position: number, zones: CriticalZone[]): number {
  const samples = [
    ...
    ...zones.map((zone) => ({
      ...
      rank: severityRank[zone.severity],
    })),
  ];
  let weightedSum = 0;
  let weightTotal = 0;
...
function buildStays(towerZ: number): Stay[] {
  ...
  [-1, 1].forEach((dirSign) => {
    for (let i = 0; i < nCables; i++) {
      ...
      const anchorZ = Math.min(
        halfSpan,
        Math.max(-halfSpan, towerZ + dirSign * reach),
      );
  ...
  return (
    <group>
      {[-1, 1].map((side) => (
        <group key={side}>
          <mesh
            position={[side * (MEDIAN_WIDTH / 2 + halfW / 2), DECK_LEVEL, 0]}
            receiveShadow
            castShadow
          >
          ...
          <mesh
            position={[(side * DECK_WIDTH) / 2, DECK_LEVEL + 2.15, 0]}
            castShadow
          >
...
function Pylon({
  ...
  return (
    <group>
      <mesh
        position={[0, config.groundY + shaftHeight / 2, config.z]}
        castShadow
      >
          ...
          <group key={index}>
            ...
            <mesh position={[0, DECK_LEVEL + 1.9, stay.anchorZ]}>
              ...
              <meshStandardMaterial
                color={color}
                emissive={color}
                emissiveIntensity={0.6 + score * 0.3}
              />
            </mesh>
          </group>
        );
...
function ResponseMarkers({ zones, monitoringPoints }: Props) {
      ...
      {zones.map((zone) => {
        ...
        return (
          ...
          >
            <mesh>
              <sphereGeometry
                args={[zone.severity === "critical" ? 1.7 : 1.2, 24, 16]}
              />
              <meshStandardMaterial
                color={severityColor[zone.severity]}
                emissive={severityColor[zone.severity]}
      ...
      {monitoringPoints.map((point, index) => {
        ...
        return (
          <group
            ...
            position={[DECK_WIDTH / 2 + 3.2, DECK_LEVEL + 3.4, z]}
          >
            <mesh>
      ...
      {selected && (
        <group
          position={[
            0,
            DECK_LEVEL + 9.5,
            (selected.position - 0.5) * SPAN_LENGTH,
          ]}
        >
          <Text
            fontSize={1.5}
...
export function BridgeScene3D({ zones, monitoringPoints }: Props) {
  return (
    <div className="scene-3d" aria-label="Modelo 3D interactivo del puente">
      <Canvas shadows dpr={[1, 2]}>
        ...
        <ResponseMarkers zones={zones} monitoringPoints={monitoringPoints} />
        <OrbitControls
          enablePan
          enableZoom
          enableRotate
      ...
      <div className="scene-help">
        <span>Arrastra</span> orbita · <span>rueda</span> zoom · el color de
        cada tirante refleja la severidad simulada en ese punto
      </div>
    </div>
  );
}
